...
description: Automates fetching unresolved pull request review comments and review threads using `gh` CLI, addressing requested changes directly in the codebase, verifying updates with tests, posting inline replies back to each PR review thread (addressed vs skipped), and generating a markdown summary report. Triggers whenever the user requests to resolve PR review comments, address PR review comments, handle code review feedback, reply to PR comments, resolve pull request feedback, or fix PR review items in a GitHub repository.
---
...
- **Reply Formatting Standard**:
  - All inline replies MUST include an AI attribution footer separated by a horizontal rule (`---`) indicating which AI agent generated the reply (e.g., `_Reply generated by Antigravity_` or `_Reply generated by Claude_`).
  - **Resolved Status**:
    ```markdown
    ...
    _Reply generated by <agent_name>_
    ```
  - **Skipped Status**:
    ```markdown
...
- **Thread Resolution**: Do NOT automatically mark review threads as resolved via API. Replying to each thread with action status and verification rationale allows reviewers and PR authors to review the updates and manually resolve threads on GitHub.
- Present a Markdown summary report to the user:
  ...
  - **Summary Table**:
    ...
    | :--- | :--- | :--- | :--- | :--- |
    | `<path>` | `<line>` | `@<author>` | `Addressed` / `Skipped` | `<summary>` |
...
_Reply generated by Antigravity_"
```
